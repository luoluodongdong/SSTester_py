...
def validate_serial_number(sn):
    ...
    error_msg = None
    reply = IPReply(IP_validateSerialNumber(uut, str(sn)))
    if not reply.is_success():
        error_msg = str(reply)
        #this reply does not need to be destroyed, because the destructor of IPReply takes care of it
    try:
        IP_UUTCancel(uut)
        IP_UID_destroy(uut)
    except Exception:
    ...
    if error_msg:
        return False  #we are not returnning the error message here. The user should be able to see it from the InstantPudding log
    else:
        return True
...
if __name__=='__main__':
    ...
    result = IPTestResult(IP_FAIL, -1,'value out of range')
    uut.add_result(spec, result)
